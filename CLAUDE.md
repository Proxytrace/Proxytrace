...
| [`docs/debug_api.md`](docs/debug_api.md) | Using/adding a debug-build-only affordance (e.g. the always-available `debug@proxytrace.dev` login) |
...
- **Backend tests** — before writing or modifying any backend test you MUST invoke the `test` skill and follow it; it is the source of truth for the harness. See [`docs/testing.md`](docs/testing.md).
...
- **File issues for stumbles** — when you hit a bug, technical debt, or other problem that is out of
  ...
  letting it slide. Invoke the `file-issue` skill — it covers
