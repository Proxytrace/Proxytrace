...
/// </summary>
public record TestRunTotals(decimal? CostUsd, long? TokensIn, long? TokensOut)
{
    public static TestRunTotals From(ITestRun run)
    {
        var usages = run.TestResults
            ...
            .ToArray();
        if (usages.Length == 0)
            return new TestRunTotals(null, null, null);
        ...
        var cost = run.Endpoint.CalculateCost(new TokenUsage(tokensIn, tokensOut));
        return new TestRunTotals(cost, (long)tokensIn, (long)tokensOut);
    }
}
