...
Scope: every backend project in `Trsr.sln` (Api, Application, Infrastructure, Storage, Domain, Serialization, Common). I cross-referenced production source against the seven `*.Tests` projects and the most recent cobertura coverage reports in `TestResults/`.
...
### 1. `OpenAiProxyController` streaming path (`Trsr.Api/Controllers/OpenAiProxyController.cs`, 318 lines)
...
### 2. `OpenAiCallParser` (`Trsr.Application/Ingestion/Internal/OpenAiCallParser.cs`, **547 lines**)
...
### 3. Statistics pipeline (`Trsr.Application/Statistics/Internal/`)
...
- `TestRunStatsStore` (`Trsr.Storage/Internal/Statistics/TestRunStatsStore.cs`, 177 lines) — no tests, although the sibling `AgentCallStatsQueriesTests` and `EvaluatorStatsQueriesTests` exist.
...
### 4. `TestRunnerService` (`Trsr.Application/TestRun/Internal/TestRunnerService.cs`, 290 lines)
...
### 5. Search subsystem (`Trsr.Application/Search/Internal/`)
...
### 6. Optimizer composition (`Trsr.Application/Optimization/Internal/`)
...
### 7. `PlaygroundService` (`Trsr.Application/Playground/Internal/PlaygroundService.cs`, 237 lines)
...
### 9. `AbstractRepository<T1,T2>` (`Trsr.Storage/Internal/AbstractRepository.cs`, 403 lines)
...
`Trsr.Infrastructure` is small (3 files, ~465 LoC) and `ModelClientTests` + `ProviderClientTests` exist. Sanity-check before release: error/retry/timeout paths in `ProviderClient` and `ModelClient` (timeouts, 429s, malformed provider responses).
...
- `TraceBroadcaster` / `TestResultBroadcaster` / `ProposalBroadcaster` — TraceBroadcaster and TestResultBroadcaster have tests; `ProposalBroadcaster` (in `Trsr.Application/Streaming/Internal/`) does **not**. Same SSE shape, cheap to add.
...
- `/Users/eberharter/Trsr/Trsr.Application/Ingestion/Internal/OpenAiCallParser.cs`
- `/Users/eberharter/Trsr/Trsr.Api/Controllers/OpenAiProxyController.cs` (streaming branch starts line 167)
- `/Users/eberharter/Trsr/Trsr.Application/Search/Internal/IndexingRepositoryDecorator.cs`
- `/Users/eberharter/Trsr/Trsr.Application/Statistics/Internal/StatisticsService.cs`
- `/Users/eberharter/Trsr/Trsr.Application/Optimization/Internal/SwitchModelOptimizer.cs`
- `/Users/eberharter/Trsr/Trsr.Application/Optimization/Internal/CompositeOptimizer.cs`
- `/Users/eberharter/Trsr/Trsr.Storage/Internal/Statistics/TestRunStatsStore.cs`
