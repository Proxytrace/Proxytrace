...
- **SSE patches the cache; it does not trigger refetches.** Per DESIGN.md §8 and the `event-stream.ts` broadcasters, live updates use `queryClient.setQueryData` to patch the cached query. Never refetch a page on an SSE event. **On an infinite query this is not a preference but a correctness rule:** `resetQueries` discards the loaded pages, leaving the query with no data until its refetch lands — and a list with no rows renders its loading skeleton, so the whole table flashes away and redraws on every event. When the event payload is too thin to patch from (`TraceCreatedEvent` carries ids, not rows), fetch the *head chunk only* and fold it in — `features/traces/traceHeadMerge.ts` + `hooks/useTraceHeadMerge.ts` is the worked example, including the trailing coalesce window that keeps a burst to one request.
...
- **Long lists must virtualize or paginate.** Never render thousands of rows. Which of the two depends on whether the list is *bounded*:
  - **Unbounded, continuously read** (the trace lists) → **virtualize**. `TraceTable` pairs `useInfiniteQuery` with `@tanstack/react-virtual`: one virtual item per row, dynamic `measureElement` (conversation groups change height when expanded), and the next chunk fetched off the virtualizer's last rendered index rather than a separate `IntersectionObserver`. A virtualized list needs a **bounded** scroll container at *every* breakpoint — if it grows to its content height, every row counts as visible and the load-more trigger never stops firing (DESIGN.md §4). Offset paging plus a list that grows at the head also means the loaded chunks can overlap, so **dedupe by id when flattening the pages** (`dedupeById`) — otherwise the overlap renders twice under a duplicate key. A deep link into a virtualized list has to **resolve against the loaded rows, then load until it can**: `querySelector` cannot find an unrendered row, and an id outside the fetched chunks is not "absent", just not here yet. `traceFocus.ts` + `hooks/useScrollToTrace.ts` is the worked example — scroll when found, pull the next chunk when not, and give up on a **bounded** budget so an unreachable link costs a fixed number of requests instead of walking the table.
