...
**Layout:** `Shell`, `Sidebar`, `Topbar`, `NavItem`, `ProjectSelector` (`components/layout/`).
...
- **Boolean** → `Switch` (on/off) or `Checkbox`; **one-of-N** → `Radio`/`RadioGroup`, or `SegmentedControl` for a compact toggle bar. On a `Switch`/`SwitchPill` both the **track and the knob are round** (`rounded-full`) — the switch is the system's one sanctioned pill silhouette (§2.3). Don't square either half.
  - **Pick between the two by what the options mean, not by how much room you have.** A `SegmentedControl` reads as *interchangeable views of one thing* — a filter, a view toggle, a unit switch — where flipping between them is free and reversible. A `RadioGroup` reads as *alternatives with different consequences*, where the user is meant to read before choosing. A consequential fork dressed as a segmented control is the mistake: it hides that the options are not symmetrical.
  - **A consequential option carries its consequence, always visible.** Give each `Radio` a rich `label` (title on the first line, one plain sentence under it) and `align="start"` so the marker sits on the title rather than floating between the lines. Revealing an option's cost only *after* it is selected turns a choice into a discovery — the judge picker in the Generate-tests panel (`synthesis/EvaluatorSuggestionCard.tsx`) is the worked example. Use `text-title` for the option title, `text-body-sm text-secondary` for the consequence, and `text-warn` only where an option reaches beyond the thing being edited.
  - `Radio` takes a `testId` (like `Segment.testId`) and `RadioGroup` takes an `ariaLabel` for when no visible heading names the choice.
