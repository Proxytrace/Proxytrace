...
export function DetailPanel({ onClose, onPrev, onNext, keyboardEnabled = true, testId, children }: DetailPanelProps) {
  ...
  return (
    <>
      <div onClick={onClose} className="fixed inset-0 z-50 bg-black/[0.4]" />
      <div
        data-testid={testId}
        className="fixed top-[76px] right-[10px] bottom-[10px] w-[min(720px,92vw)] bg-card rounded-xl flex flex-col overflow-hidden z-[51] shadow-[var(--shadow-float)] [animation:fade-up_0.25s_cubic-bezier(0.2,0.8,0.2,1)]"
      >
        {children}
      </div>
    </>
