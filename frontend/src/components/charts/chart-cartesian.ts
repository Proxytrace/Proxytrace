import { type GridLine, buildGridPaths } from './chart-scale';
import { BAR_PAD_L, BAR_PAD_R, barBand, barsRectPath } from './chart-bars';
...
export function computeHistogram(
  ...
  const padL = BAR_PAD_L, padR = BAR_PAD_R, padT = 10, padB = 24;
  const w = width - padL - padR, h = height - padT - padB;
  const max = Math.max(...data, 0) * 1.1 || 1;
  const { bw, x } = barBand(w, data.length, padL, 0.86, 0.14);
...
export function computeModelBars(
  ...
  const padL = BAR_PAD_L, padR = BAR_PAD_R, padT = 10, padB = 36;
  const w = width - padL - padR, h = height - padT - padB;
  const max = Math.max(...data.map(d => d.value)) * 1.1 || 1;
  const { bw, x } = barBand(w, data.length, padL, 0.7, 0.3);
