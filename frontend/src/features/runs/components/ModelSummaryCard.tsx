    ...
    <Card padding="none">
      ...
      <div className="p-3 pl-[18px]">
        <div className="flex items-center justify-between gap-2 flex-wrap mb-2">
          ...
          {multi && (
            <div className="flex gap-1 flex-wrap justify-end">
              {entry.isBest && <WinnerBadge tone="best" label={t`Best`} icon={<TargetIcon size={9} />} />}
              {entry.isFastest && <WinnerBadge tone="fast" label={t`Fast`} icon={<ZapIcon size={9} />} />}
              {entry.isCheapest && <WinnerBadge tone="cheap" label={t`Cheap`} icon={<CoinsIcon size={9} />} />}
            </div>
          )}
        </div>
        ...
        <div className="flex gap-3 mb-2.5 mono text-body-sm">
          ...
          {pending > 0 && <span className="text-muted font-semibold">{pending}<span className="font-normal"> <Trans>pending</Trans></span></span>}
        </div>
