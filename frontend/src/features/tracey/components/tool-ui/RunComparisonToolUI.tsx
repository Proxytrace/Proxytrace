...
const MOVEMENT_LABEL: Record<Exclude<CaseMovement, 'still-passing'>, { label: string; variant: 'success' | 'danger' | 'neutral' }> = {
  fixed: { label: 'Fixed', variant: 'success' },
  regressed: { label: 'Regressed', variant: 'danger' },
  'still-failing': { label: 'Still failing', variant: 'neutral' },
};
...
export const RunComparisonToolUI: ToolCallMessagePartComponent = ({ result, status, isError }) => {
  ...
  return (
    <ToolUIFrame
      ...
      title={data ? `Run comparison · ${data.suiteName ?? ''}` : 'Run comparison'}
      pendingLabel="Comparing runs…"
    ...
    >
      {data && (
        <div className="flex flex-col gap-3">
          ...
          <StatGrid
            items={[
              { label: 'Fixed', value: String(data.fixed) },
              { label: 'Regressed', value: String(data.regressed) },
              { label: 'Still failing', value: String(data.stillFailing) },
            ]}
          />
          {moved.length > 0 && (
            <div className="flex flex-col divide-y divide-border-subtle">
              {moved.map((item) => (
                <div key={item.testCaseId} className="flex items-center gap-2.5 py-1.5 first:pt-0 last:pb-0">
                  ...
                  <Badge
                    label={MOVEMENT_LABEL[item.movement as keyof typeof MOVEMENT_LABEL].label}
          ...
          {data.unmatched > 0 && (
            <div className="text-body-sm text-muted">
              {data.unmatched} case{data.unmatched === 1 ? '' : 's'} not comparable (suite changed between runs).
            </div>
          )}
        </div>
      )}
