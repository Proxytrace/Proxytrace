 ...
 *   {@link migrateLegacyThread} can fold a pre-history conversation into the new model.
 * - The **conversation history** — an index of up to {@link MAX_CONVERSATIONS} conversation
 *   metadata records (`proxytrace.tracey.conversations:{user}:{project}`) plus one snapshot blob
 *   per conversation (`proxytrace.tracey.conversation:{user}:{project}:{id}`). See the conversation
 *   section below.
 */
