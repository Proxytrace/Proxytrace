...
describe('readMessageStats', () => {
  ...
  it('reads usage and duration', () => {
    ...
    ).toEqual({ inputTokens: 14200, cachedInputTokens: 0, outputTokens: 226, totalTokens: 14426, durationMs: 2300 });
  });
  ...
  it('reads cachedInputTokens from usage', () => {
    expect(
      readMessageStats({ usage: { inputTokens: 1000, cachedInputTokens: 600, outputTokens: 50 } }),
    ).toEqual({ inputTokens: 1000, cachedInputTokens: 600, outputTokens: 50, totalTokens: 1050, durationMs: null });
  });

  it('defaults cachedInputTokens to 0 when absent from usage', () => {
    const stats = readMessageStats({ usage: { inputTokens: 1000, outputTokens: 50 } });
